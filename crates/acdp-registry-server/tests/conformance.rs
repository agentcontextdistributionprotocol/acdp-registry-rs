...
//! COVERED in Phase 11; `schema` in Phase 12, same #130 filing).
...
/// and the other six (`did-ssrf`, `dk`, `err`, `rate`, `rev`, `sig`)
/// are currently `DEFERRED` -- both groups are spec-required all
...
/// (`schema_vectors_openness_and_absent_vs_null_enforced`). The remaining 10 cite
